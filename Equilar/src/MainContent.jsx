...
const MainContent = () => {
  ...
  const renderEmployee = (person, type) => (
      ...
      <div className="name-date-row">
        ...
        <div className="update-date">
          ...
          {/* {person.source?.source && (
            <span className="source"> ({person.source.source})</span>
          )} */}
        </div>
      </div>
...
const renderCompany = (company, index) => {
      ...
      {Object.entries(events).map(([eventType, people]) =>
        people.length > 0 ? (
         ...
         <p className="event-type-title" style={{ color: colorMap[eventType] || '#000' }}>
          ...
          {people.map(person => renderEmployee(person, eventType))}
         </div>
        ) : null
      )}
    </div>
         ...
         <div className="all-updates-container">
            {updatesData.companyUpdatesList.map((company, index) => (
              <div key={index}>
                ...
                {index < updatesData.companyUpdatesList.length - 1 && <hr className="company-divider" />}
              </div>
            ))}
         </div>
  ...
  <div className="sidebar-header">
    <span className="sidebar-title">EMAIL PREFERENCES</span>
